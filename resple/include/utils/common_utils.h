...
public:
    ...
    // Attitude helpers now live in the ROS-free numeric core
    // (utils/geometry_core.h) so the gravity-init math is unit-tested; these
    // forward to keep every existing call site (CommonUtils::R2ypr/ypr2R/g2R)
    // working unchanged.
    static Eigen::Vector3d R2ypr(const Eigen::Matrix3d &R)
    {
        return resple::geom::r2ypr(R);
    }
    ...
    static Eigen::Matrix<typename Derived::Scalar, 3, 3> ypr2R(const Eigen::MatrixBase<Derived> &ypr)
    {
        ...
        const Eigen::Vector3d ypr_d(static_cast<double>(ypr(0)),
                                    static_cast<double>(ypr(1)),
                                    static_cast<double>(ypr(2)));
        return resple::geom::ypr2r(ypr_d).cast<Scalar_t>();
    }

    static Eigen::Matrix3d g2R(const Eigen::Vector3d &g)
    {
        return resple::geom::g2r(g);
    }
