...
| Degenerate-direction gate (X-ICP-style) | `loc_gate_trans_min_eig` | 0 (off) | Prototype VALIDATED 2026-06-11 @0.02: tunnel map-lock oscillation eliminated (max step 16.9 m → 0.18 m), HelmDyn01 APE unchanged despite 35 % arming. Replicated sweep 2026-06-12 (n=3 × {0,0.02,0.05,0.10}): **0.10 safest** — 0/3 catastrophic teleports (bounded <3.8 m) vs 1/3 at 0.02/0.05, no HelmDyn regression (APE 4.65–4.90 vs 4.88–4.93 control); but no clean standalone (residual jitter; gentle pull gate-insensitive). Before default: relative/condition arming + GT-scored tunnel | §3.2 / doc/PARAMETERS.md |
| Publish-side cov inflation along gated axes | `loc_gate_cov_rate` (+`_decay`/`_persist`/`_min_eig`) | 0 (off) | Advisory inflation of the **published** `/odom` pose covariance so a downstream EKF down-weights RESPLE along a tunnel axis (the raw IEKF posterior reports only cm-scale local uncertainty there). Publish-side only — filter/trajectory byte-identical. VALIDATED 2026-06-12: tunnel along-axis std 3.7 cm → ~30 cm (cond# → 70+), smooth ground (R_Campus) stays at baseline (~1 cm); aggressive *handheld* (HelmDyn) false-positives (~12 cm) — the trigger reuses the gate's per-frame eigenvalue, which is non-specific to true degeneracy (same limit motivating relative arming). Pairs with the gate; default-off pending target-platform validation | §6.7 / doc/PARAMETERS.md |
